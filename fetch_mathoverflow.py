...
def fetch_mathoverflow(dataset, test_set_fraction=0.2,
    ...
    if dataset not in ('mathoverflow',):
    ...
    if min_training_interactions > 0:
        ...
        pd.DataFrame(np.where(include)[0]).to_csv("user_ids_kept.csv",index=False,header=False)
